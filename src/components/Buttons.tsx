...
import { GoogleSVG, GitHubSVG } from "./SVGs";
...
export function SignInButton({ providerId, providerName }: SignInButtonProps) {
  ...
  return (
    <button
      onClick={() => signIn(providerId, { callbackUrl: "/about" })}
      className={`rounded-full flex items-center justify-center py-4 px-5 text-white w-full ${bgClass}`}
    >
